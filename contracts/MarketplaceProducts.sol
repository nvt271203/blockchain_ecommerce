...
contract MarketplaceProducts {
    ...
    uint public count = 0;  // Đếm số lượng sản phẩm trong smart contract
//The product Details
    struct Product {
        uint id; // 1.2.3...
        string name;
        string description;
        string image; // lưu ảnh lên ipfs
        bool sold; // false(mặc định)
        address payable owner; // chủ sở hữu - 0xádlfdsal;kdsfaj
        uint price; // Giá Wei = 10 mũ 18
        string category;
    }
