...
class ContractFactoryServies extends ChangeNotifier {
  ...
  List<ProductModel> purchasedProducts = []; // Sản phẩm đã mua (dùng cho OrdersScreen)
  //
...
// GET PURCHASED PRODUCTS OF THE CURRENT USER FROM BLOCKCHAIN
//   Future<void> _getUserPurchasedProducts() async {
//     try {
//       if (_productCount == null || myAccount == null) {
//         print("Lỗi: _productCount hoặc myAccount chưa được khởi tạo.");
//         storeProductsLoading = false;
//         notifyListeners();
//         return;
//       }
...
//         );
//
//         // Kiểm tra điều kiện: owner là myAccount và sold là true
//         if (product[4] == true && product[5].toString().toLowerCase() == myAccount!.toLowerCase()) {
//           ProductModel productModel = ProductModel(
//             id: product[0],
//             name: product[1],
//             description: product[2],
...
//             category: product[7],
//           );
//           print('Sản phẩm đã mua: ${productModel.toString()}');
//           allProducts.add(productModel);
//         }
//       }
//
//       storeProductsLoading = false;
//       print("Số lượng sản phẩm đã mua: ${allProducts.length}");
//     } catch (e) {
//       storeProductsLoading = false;
//       print("Lỗi khi lấy sản phẩm đã mua: $e");
//     }
//     notifyListeners();
//   }
  ...
  // GET PURCHASED PRODUCTS OF THE CURRENT USER FROM BLOCKCHAIN
  Future<void> getUserPurchasedProducts(String accout) async {
    try {
      if (_productCount == null || accout == null) {
        print("Lỗi: _productCount hoặc myAccount chưa được khởi tạo.");
        storeProductsLoading = false;
        notifyListeners();
        return;
      ...
      int count = int.parse(_productCount.toString());
      purchasedProducts.clear(); // Xóa danh sách sản phẩm đã mua cũ
      ...
      for (int i = 1; i <= count; i++) {
        ...
        // Kiểm tra điều kiện: owner là myAccount và sold là true
        if (product[4] == true && product[5].toString().toLowerCase() == accout!.toLowerCase()) {
          ProductModel productModel = ProductModel(
            id: product[0],
            name: product[1],
            description: product[2],
          ...
          print('Sản phẩm đã mua: ${productModel.toString()}');
          purchasedProducts.add(productModel);
        }
      }

      storeProductsLoading = false;
      print("Số lượng sản phẩm đã mua: ${purchasedProducts.length}");
    } catch (e) {
      storeProductsLoading = false;
      print("Lỗi khi lấy sản phẩm đã mua: $e");
    }
  ...
  getUserProducts(String addressWallet) async {
    try {
      ...
      for (int i = 1; i <= count; i++) {
        ...
        if (product.isNotEmpty && product[5].toString() == addressWallet && product[4] != true) {
