...
class _AccoutScreenState extends State<AccoutScreen> {
  ...
  final String defaultWalletAddress = 'Trống';
  String balance = '0.0 ETH';

  String? chainName;
  String? walletAddress;
  bool isLoading = true; // Thêm biến trạng thái loading
  final ContractFactoryServies _contractFactoryServies =
      ...
      if (value != null) {
        setState(() {
          ...
          _contractFactoryServies
              .getBalance(walletAddress.toString())
              .then((value) {
            setState(() {
              balance = value;
  ...
  void disconnectWallet() async {
    await _contractFactoryServies
        .disconnect(); // Gọi hàm ngắt kết nối của WalletConnect
    setState(() {
      walletAddress = null;
      balance = '0.0 ETH';
  ...
  Widget build(BuildContext context) {
    ...
    // contractFactory.restoreWalletConnection();
                  ...
                  child: Column(
                    children: [
                      isLoading
                          ? const SizedBox(
                              height: 140,
                              // Chiều cao bằng 2 * radius của CircleAvatar
                              child: Center(
